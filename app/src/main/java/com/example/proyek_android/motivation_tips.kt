...
class motivation_tips : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_motivation_tips)

        sp = getSharedPreferences("dataSP", MODE_PRIVATE)
        db = FirebaseFirestore.getInstance()
        rvArticle = findViewById(R.id.rvMotivation)
    ...
    private fun TampilkanData() {
        ...
        db.collection("articles")
            .whereEqualTo("article_category","Motivation Tips")
            .get()
        ...
        adapterArt.setOnItemClickCallback(object: adapterArticle.OnItemClickCallBack {
            override fun onItemClicked(data: Articles){
                Toast.makeText(this@motivation_tips, data.article_title, Toast.LENGTH_LONG).show()
            }
        })
    }
}